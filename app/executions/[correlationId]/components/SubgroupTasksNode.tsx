...
function SubgroupNode({ record, onClick, isSelected }: { record: TaskStatusRecord, onClick?: (record: TaskStatusRecord) => void, isSelected: boolean }) {
    ...
    return (
        <div className={`inline-flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all ${isSelected ? 'bg-blue-100 border-blue-500' : 'bg-gray-100 border-gray-300'}`} onClick={() => { onClick && onClick(record) }}>
