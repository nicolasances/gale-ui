...
export function StatusBadge({ status }: { status: TaskStatus }) {
    ...
    return (
        <div className={`flex ${colors[status]} w-6 h-6 justify-center items-center rounded-full`}>
            {status === 'completed' ? (
                <svg className="w-2 h-2 inline" viewBox="0 -3 32 32" fill="currentColor">
                ...
                <svg className="w-2 h-2 inline" viewBox="0 0 512 512" fill="currentColor">
